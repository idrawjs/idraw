...
import type { PointWatcherEvent } from './watcher';
...
export type BoardWatcherPointEvent = PointWatcherEvent;
export type BoardWatherWheelXEvent = PointWatcherEvent & {
  deltaX: number;
};
export type BoardWatherWheelYEvent = PointWatcherEvent & {
  deltaY: number;
};
export type BoardWatherWheelEvent = PointWatcherEvent & {
  deltaX: number;
  deltaY: number;
};
...
export interface BoardMiddlewareObject<S extends Record<any | symbol, any> = any, C extends any = undefined> {
  ...
  click?: (e: BoardWatcherEventMap<S>['click']) => void | boolean;
